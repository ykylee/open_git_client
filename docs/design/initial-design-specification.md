...
- `SRS-006`부터 `SRS-013`까지의 플랫폼 및 기술 제약에 따라 `C++20 + wxWidgets + CMake + libgit2 + native git CLI`를 유지한다.
...
### ADR-001 UI 계층은 `wxWidgets`로 시작한다
- 결정: Release 1 UI는 `wxWidgets` 기반으로 설계한다.
- 이유: 회사 환경에서 무료 사용이 가능한 C++ 중심 데스크톱 프레임워크가 필요하고, Windows 우선이면서 Linux 빌드 유지가 가능해야 한다.
- 결과: 그래프 뷰와 diff 뷰는 커스텀 위젯 기반으로 확장하고, 웹 기반 UI 계층 도입은 Release 1 범위에서 제외한다.
...
## 변경 이력
### 개발 버전 `v0.0.2`
- 2026-03-12: 현재 프로젝트 개발 버전 `v0.0.2`에 맞춰 문서의 기준 개발 버전과 변경 이력 표기를 갱신하고 Release 1 UI 계층을 `wxWidgets` 기준으로 정리했다.
