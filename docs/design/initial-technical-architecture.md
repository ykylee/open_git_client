...
- 스레드 풀 또는 동등한 작업 실행 계층으로 백그라운드 작업을 처리한다.
...
- Windows CI는 `MinGW-w64`, Linux CI는 `GCC` 또는 `Clang` 매트릭스로 운영한다.
...
## 변경 이력
### 개발 버전 `v0.0.2`
- 2026-03-12: 현재 프로젝트 개발 버전 `v0.0.2`에 맞춰 문서의 기준 개발 버전과 변경 이력 표기를 갱신하고 UI 프레임워크를 `wxWidgets`, Windows 주 개발 도구와 CI 기준을 `MinGW-w64`로 정리했다.

### 개발 버전 `v0.0.1`
- 2026-03-11: 초기 기술 아키텍처 초안을 작성하고 SRS v0.6 기준 Release 1 범위, 계층 구조, 백엔드 전략, 성능 중심 설계 원칙을 반영했으며 변경 이력 표기를 개발 버전 기준으로 맞췄다.
